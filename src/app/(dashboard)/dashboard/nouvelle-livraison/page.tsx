...
export default function NouvelleLivraison() {
  ...
  const handleConfirmerPaiement = async () => {
    // ✅ V1 : Validation opérateur + numéro
    ...
    return (
      <div className="min-h-screen bg-[#F9FAFB] flex flex-col items-center justify-center p-6">
        ...
        <div className="grid grid-cols-3 gap-3 w-full max-w-sm">
          ...
          >
            <Smartphone size={28} />
            <span className="text-xs font-semibold text-center leading-tight">Commande automatique</span>
          </button>
          ...
          >
            ...
            <span className="text-xs font-semibold text-center leading-tight">Commander par message</span>
          </button>
          ...
          >
            <Phone size={28} />
            <span className="text-xs font-semibold text-center leading-tight">Commander par appel</span>
          </button>
        </div>

        <p className="text-xs text-[#6B7280] mt-6">Service disponible de 08h à 20h</p>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen bg-[#F9FAFB] flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-md p-6 w-full max-w-sm text-center space-y-6">
          ...
          <p className="text-sm text-[#6B7280]">Transmettez ces codes à votre livreur</p>

          <div className="grid grid-cols-2 gap-3">
            <div className="bg-[#FFF4EF] rounded-2xl p-4">
              <p className="text-xs text-[#6B7280] mb-1">Code départ</p>
              <p className="text-3xl font-bold tracking-widest text-[#FF6B35]">{codes.depart}</p>
            </div>
            <div className="bg-[#F0FDF4] rounded-2xl p-4">
              <p className="text-xs text-[#6B7280] mb-1">Code livraison</p>
              <p className="text-3xl font-bold tracking-widest text-green-600">{codes.livraison}</p>
            </div>
          </div>

          <p className="text-xs text-[#6B7280]">⚠️ Gardez ces codes jusqu'à la livraison</p>

          {/* ✅ V2 : Reset complet au retour */}
          <Button
            onClick={() => {
    ...
    <div className="min-h-screen bg-[#F9FAFB] px-4 py-6">
      ...
      <form onSubmit={handleSoumettreFormulaire} className="space-y-4">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              className="h-12 rounded-xl border-gray-200 bg-[#F9FAFB] focus:bg-white focus:border-[#FF6B35] transition-all"
            />
          </div>

          {/* ✅ V2 : Champ téléphone avec préfixe +226 et compteur */}
          <div className="space-y-1.5">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 space-y-4">
          ...
          {/* ✅ V1 : Bouton désactivé si formulaire invalide */}
          <Button
            type="submit"
            disabled={!formValide}
            className="w-full h-12 rounded-xl font-semibold transition-all active:scale-95 disabled:bg-gray-200 disabled:text-gray-400 enabled:bg-[#FF6B35] enabled:hover:bg-[#e55a25] enabled:text-white"
          >
      ...
      {/* ✅ V2 : Modal paiement avec animation + V1 : choix opérateur */}
      {etape === 'paiement' && (
        <div className="fixed inset-0 bg-black/50 flex items-end justify-center z-50">
          <div className="bg-white w-full max-w-md p-6 rounded-t-3xl space-y-5 animate-in slide-in-from-bottom">
            ...
            {/* ✅ V1 : Sélection opérateur */}
            <div className="space-y-1.5">
              <Label className="text-sm font-medium text-[#0A0A0A]">Opérateur</Label>
              <div className="grid grid-cols-2 gap-3">
                {OPERATEURS.map(op => (
                  <button
                    key={op.value}
                    ...
                    className={`h-12 rounded-xl border-2 text-sm font-semibold transition-all
                      ${paiement.operateur === op.value
                        ...
                        : 'border-gray-200 bg-[#F9FAFB] text-[#6B7280]'
                      }`}
                  >
                    {op.label}
                  </button>
            ...
            <div className="flex gap-3 pt-2">
              <Button variant="outline" onClick={() => setEtape('formulaire')} className="flex-1 h-12 rounded-xl">
                Retour
              </Button>
              <Button
